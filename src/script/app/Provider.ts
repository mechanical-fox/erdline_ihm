...
/**
 *  This class provide some function.
 *  Particulary it will be provide function than must be mocked during the tests.
 *
 *  And this class allow to replace the functions provided by a mock
 */
export class Provider {
    
    static mockForFetch: ((url: string, options: Fetch_Options) => Promise<Fetch_Response>) | null = null;
    ...
    static fetch(url: string, options: Fetch_Options): Promise<Fetch_Response> {
        if (this.mockForFetch == null) return fetch(url, options);
